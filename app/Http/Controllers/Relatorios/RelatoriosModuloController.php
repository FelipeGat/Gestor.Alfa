...
class RelatoriosModuloController extends Controller
{
    ...
    private function montarDados(Request $request): array
    {
        ...
        $empresaPadrao = $empresas->first()?->id;

        $filtros = [
            'empresa_id' => (int) ($request->input('empresa_id', $empresaPadrao) ?: 0),
            'data_inicio' => $request->input('data_inicio', now()->startOfMonth()->toDateString()),
            'data_fim' => $request->input('data_fim', now()->endOfMonth()->toDateString()),
