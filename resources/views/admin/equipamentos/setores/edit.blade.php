<x-app-layout>
    ...
    <x-slot name="breadcrumb">
        <x-breadcrumb-tabs :items="[
            ...
            ['label' => 'Setores', 'url' => route('admin.equipamentos.setores.index')],
            ['label' => 'Editar Setor']
        ]" />
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-lg border border-gray-200">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h2 class="text-lg font-semibold text-gray-900">Editar Setor</h2>
                ...
                <form action="{{ route('admin.equipamentos.setores.update', $setor->id) }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <select id="cliente_id" name="cliente_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                            ...
                            @foreach($clientes as $cliente)
                                <option value="{{ $cliente->id }}" @selected(old('cliente_id', $setor->cliente_id) == $cliente->id)>
                    ...
                    <div>
                        ...
                        <x-text-input id="nome" name="nome" type="text" class="mt-1 block w-full" :value="old('nome', $setor->nome)" required autofocus />
                        <x-input-error :messages="$errors->get('nome')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="descricao" value="Descrição" />
                        <textarea id="descricao" name="descricao" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('descricao', $setor->descricao) }}</textarea>
