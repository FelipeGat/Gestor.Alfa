...
<div class="rounded-md p-4 border {{ $typeData['bg'] }} {{ $typeData['border'] }} {{ $typeData['text'] }}" role="alert">
    <div class="flex">
        ...
        <div class="ml-3 flex-1">
            @if($title)
                <h3 class="text-sm font-semibold {{ $typeData['title_color'] }}">
                    {{ $title }}
                </h3>
            @endif
            ...
            @if($dismissible)
                <div class="mt-4">
                    <button type="button" onclick="this.parentElement.parentElement.parentElement.remove()" class="text-sm underline hover:text-{{ $type }}-900">
                        Dismiss
                    </button>
                </div>
            @endif
