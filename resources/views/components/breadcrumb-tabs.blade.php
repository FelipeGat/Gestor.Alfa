    ...
    <nav class="flex items-end gap-1 overflow-x-auto">
        @foreach ($tabs as $tab)
            ...
            <div class="tab-item group relative" data-tab-id="{{ $tabId }}" data-tab-url="{{ $tabUrl }}">
                @if ($isActive)
                    <span class="relative bg-white px-4 py-2 text-base font-semibold text-[#3f9cae]
                                 rounded-t-lg border-2 border-[#3f9cae] flex items-center gap-2">
                        {{ $tab['label'] }}
                        <button type="button" 
                                onclick="event.stopPropagation(); window.fecharTab('{{ $tabId }}')"
                                class="ml-3 w-6 h-6 flex items-center justify-center rounded-full 
                ...
                @else
                    <a href="{{ $tabUrl }}" 
                       ...
                       class="relative bg-gray-200 px-4 py-2 text-base font-semibold text-gray-600
                        ...
                        <button type="button" 
                                ...
                                class="ml-3 w-6 h-6 flex items-center justify-center rounded-full 
                                       text-gray-400 hover:text-red-500 hover:bg-red-100 
                                       transition-colors text-base leading-none opacity-0 group-hover:opacity-100">
                            ×
                        </button>
                    </a>
                @endif
