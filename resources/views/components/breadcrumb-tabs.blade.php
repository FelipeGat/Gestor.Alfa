            ...
            <div class="tab-item group relative flex-shrink-0" data-tab-id="{{ $tabId }}" data-tab-url="{{ $tabUrl }}">
                @if ($isActive)
                    ...
                    <button type="button" 
                            ...
                            class="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 flex items-center justify-center rounded-full 
                ...
                @else
                    <a href="{{ $tabUrl }}" 
                       ...
                       class="relative bg-gray-200 px-4 pr-12 py-2 text-sm font-semibold font-semibold text-gray-600 rounded-t-lg border border-gray-300 flex items-center hover:bg-gray-300 hover:text-gray-800 transition-all">
                        {{ $tab['label'] }}
                    </a>
                    <button type="button" 
                            onclick="event.preventDefault(); event.stopPropagation(); window.fecharTab('{{ $tabId }}')"
                            class="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 flex items-center justify-center rounded-full 
                                   font-semibold text-gray-600 hover:text-red-500 hover:bg-red-100 
                                   transition-colors text-sm leading-none">
                        x
                    </button>
                @endif
