<x-app-layout>
    ...
    <x-page-title title="Editar Subcategoria" :route="route('categorias.index')" />
    ...
    <div class="pb-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form action="{{ route('subcategorias.update', $subcategoria) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white rounded-lg p-6 sm:p-8" style="border: 1px solid #3f9cae; border-top-width: 4px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        <div>
                            <x-form-select name="categoria_id" label="Categoria" required placeholder="Selecione a categoria">
                                @foreach($todasCategorias as $categoria)
                                    <option value="{{ $categoria->id }}" @selected(old('categoria_id', $subcategoria->categoria_id) == $categoria->id)>
                                        {{ $categoria->nome }}
                                    </option>
                                @endforeach
                            </x-form-select>
                        ...
                        <div class="col-span-1 sm:col-span-2">
                            <x-form-input name="nome" label="Nome" required placeholder="Ex: Supermercado, Farmácia" :value="old('nome', $subcategoria->nome)" />
                        </div>

                        <div>
                            <x-form-select name="ativo" label="Status" placeholder="Selecione">
                                <option value="1" @selected(old('ativo', $subcategoria->ativo) == 1)>Ativo</option>
                                <option value="0" @selected(old('ativo', $subcategoria->ativo) === 0)>Inativo</option>
                            </x-form-select>
                        </div>
                    </div>
                </div>
