            ...
            function renderAbas() {
                ...
                const tabsHtml = tabs.map(tab => {
                    ...
                    const closeButton = isActive 
                        ? `<button type="button" onclick="event.stopPropagation(); window.fecharTab('${tab.id}')" class="ml-3 w-6 h-6 flex items-center justify-center rounded-full text-gray-400 hover:text-red-500 hover:bg-red-100 transition-colors text-base leading-none">×</button>`
                        : `<button type="button" onclick="event.preventDefault(); event.stopPropagation(); window.fecharTab('${tab.id}')" class="ml-3 w-6 h-6 flex items-center justify-center rounded-full text-gray-400 hover:text-red-500 hover:bg-red-100 transition-colors text-base leading-none opacity-0 group-hover:opacity-100">×</button>`;

                    if (isActive) {
                        return `<div class="tab-item group relative" data-tab-id="${tab.id}" data-tab-url="${tab.url}">
                            <span class="relative bg-white px-4 py-2 text-base font-semibold text-[#3f9cae] rounded-t-lg border-2 border-[#3f9cae] flex items-center gap-2">
                                ${tab.label}
                                ${closeButton}
                            </span>
                        </div>`;
                        ...
                        return `<div class="tab-item group relative" data-tab-id="${tab.id}" data-tab-url="${tab.url}">
                            <a href="${tab.url}" onclick="event.preventDefault(); window.ativarTab('${tab.id}')" class="relative bg-gray-200 px-4 py-2 text-base font-semibold text-gray-600 rounded-t-lg border border-gray-300 flex items-center gap-2 hover:bg-gray-300 hover:text-gray-800 transition-all">
                                ${tab.label}
                                ${closeButton}
                            </a>
                        </div>`;
