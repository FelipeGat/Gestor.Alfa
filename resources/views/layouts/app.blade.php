            ...
            function renderAbas() {
                ...
                const tabsHtml = tabs.map(tab => {
                    ...
                    if (isActive) {
                        ...
                        return `<div class="tab-item group relative flex-shrink-0" data-tab-id="${tab.id}" data-tab-url="${tab.url}">
                            <a href="${tab.url}" onclick="event.preventDefault(); window.ativarTab('${tab.id}')" class="relative bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-600 rounded-t-lg border border-gray-300 flex items-center hover:bg-gray-300 hover:text-gray-800 transition-all">
                                ${tab.label}
                            </a>
                            ${closeButton}
                        </div>`;
