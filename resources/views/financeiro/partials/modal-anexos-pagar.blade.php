{{-- Modal de Gerenciamento de Anexos - Contas a Pagar --}}
<div
    x-data="modalAnexosPagar()"
    x-show="modalAberto"
    style="display: none;"
    x-cloak
    class="fixed inset-0 z-50 overflow-y-auto"
    @keydown.escape.window="fecharModal()"
    @abrir-modal-anexos-pagar.window="abrirModal($event.detail)">
                    ...
                    <form @submit.prevent="uploadAnexo()" enctype="multipart/form-data">
                        <div class="space-y-4">
                            ...
                            <button
                                type="submit"
                                :disabled="enviando"
                                class="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-4 rounded-lg transition shadow-md disabled:opacity-50 disabled:cursor-not-allowed">
                        ...
                        <template x-for="anexo in anexos" :key="anexo.id">
                            <div class="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                                ...
                                <div class="flex items-center gap-2 ml-4">
                                    {{-- Botão Download --}}
                                    <a
                                        :href="`/financeiro/contas-pagar/anexos/${anexo.id}/download`"
                                        target="_blank"
    ...
    function modalAnexosPagar() {
        return {
            ...
            async carregarAnexos() {
                ...
                try {
                    const response = await fetch(`/financeiro/contas-pagar/${this.contaId}/anexos`);
            ...
            async excluirAnexo(anexoId) {
                ...
                try {
                    const response = await fetch(`/financeiro/contas-pagar/anexos/${anexoId}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
