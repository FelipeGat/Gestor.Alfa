<x-app-layout>
    ...
    <x-page-title title="Editar Categoria" :route="route('categorias.index')" />
                ...
                <div class="bg-white rounded-lg p-6 sm:p-8" style="border: 1px solid #3f9cae; border-top-width: 4px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        <div class="col-span-1 sm:col-span-2">
                            <x-form-input name="nome" label="Nome" required placeholder="Ex: Alimentação, Transporte" :value="old('nome', $categoria->nome)" />
                        </div>

                        <div>
                            <x-form-select name="tipo" label="Tipo" required placeholder="Selecione">
                                <option value="FIXA" @selected(old('tipo', $categoria->tipo) == 'FIXA')>Fixa</option>
                                <option value="VARIAVEL" @selected(old('tipo', $categoria->tipo) == 'VARIAVEL')>Variável</option>
                                <option value="INVESTIMENTO" @selected(old('tipo', $categoria->tipo) == 'INVESTIMENTO')>Investimento</option>
                            </x-form-select>
                        </div>

                        <div>
                            <x-form-select name="ativo" label="Status" placeholder="Selecione">
                                <option value="1" @selected(old('ativo', $categoria->ativo) == 1)>Ativo</option>
                                <option value="0" @selected(old('ativo', $categoria->ativo) === 0)>Inativo</option>
                            </x-form-select>
                        </div>
                    </div>
                </div>
