<x-app-layout>
    ...
    <x-page-title title="Editar Usuário" :route="route('usuarios.index')" />
            ...
            <form method="POST" action="{{ route('usuarios.update', $usuario) }}" class="space-y-6">
                ...
                <x-form-section title="Dados do Usuário">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <x-form-input name="password" label="Nova Senha" type="password" placeholder="Preencha para alterar" />
                        <x-form-select name="tipo" label="Tipo de Usuário" :selected="$usuario->tipo" :options="[
                            'admin' => 'Admin',
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        @foreach($empresas as $empresa)
                            <x-form-checkbox 
                                ...
                                :checked="$usuario->empresas->contains($empresa->id)" 
                            />
                        @endforeach
                    </div>
                </x-form-section>
